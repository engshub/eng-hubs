/* ============================================================
   ENG HUBS â Script principal (Vanilla JS)
   ============================================================ */
(function () {
    'use strict';
    ...
    const CONCURSOS = [
        {
            id: 'c1',
            orgao: 'TCE-SP',
            cargo: 'Auditor de Controle Externo â Engenharia',
        ...
        {
            id: 'c2',
            orgao: 'Prefeitura de CamboriÃº/SC',
        ...
        },
        {
            id: 'c3',
            orgao: 'TCM-SP',
            cargo: 'Auditor de Controle Externo â Arquitetura',
            area: 'arquitetura',
        ...
        },
        {
            ...
            area: 'eletrica',
            areaLabel: 'Engenharia ElÃ©trica',
            banca: 'IBFC',
            estado: 'PR',
            vagas: 3,
            salario: 9870,
        ...
        },
        {
            id: 'c5',
            orgao: 'IBAMA',
            cargo: 'Analista Ambiental â Engenharia',
            area: 'ambiental',
        ...
        },
        {
            id: 'c8',
            orgao: 'TJ-RS',
            cargo: 'Analista JudiciÃ¡rio â Engenharia Civil',
            area: 'engenharia-civil',
            areaLabel: 'Engenharia Civil',
            banca: 'FAURGS',
            estado: 'RS',
        ...
        },
        {
            id: 'c9',
            orgao: 'CAU/BR',
            cargo: 'Arquiteto e Urbanista â FiscalizaÃ§Ã£o',
            area: 'arquitetura',
            areaLabel: 'Arquitetura',
            banca: 'IADES',
            estado: 'DF',
    ...
    function renderCard(c) {
        ...
        if (dias < 0) {
            ...
            textoPrazo = `<strong>Ãltimo dia para inscriÃ§Ã£o!</strong>`;
        } else {
            textoPrazo = `InscriÃ§Ãµes atÃ© <strong>${formatData(c.inscricoesAte)}</strong> Â· ${dias} dia${dias > 1 ? 's' : ''}`;
        }
        return `
            <article class="card" data-id="${c.id}" data-area="${c.area}">
                ...
                <div class="card-info">
                    ...
                    <div class="card-info-item">
                        <span class="card-info-label">RemuneraÃ§Ã£o</span>
    ...
    function showToast({ title, message, type = 'default', duration = 4500 } = {}) {
        ...
        toast.innerHTML = `
            ...
            <div class="toast-body">
                <div class="toast-title">${title || 'NotificaÃ§Ã£o'}</div>
                ${message ? `<div class="toast-message">${message}</div>` : ''}
            </div>
            <button class="toast-close" aria-label="Fechar notificaÃ§Ã£o">
                <i data-lucide="x"></i>
            </button>
        `;
    ...
    function toggleAcompanhar(id, btn) {
        ...
        if (isAtivo) {
            ...
            showToast({
                ...
                message: concurso ? `VocÃª nÃ£o receberÃ¡ mais alertas sobre ${concurso.orgao}.` : ''
            });
        } else {
            ...
            showToast({
                ...
                message: concurso
                    ? `VocÃª receberÃ¡ alertas sobre ${concurso.orgao} â ${concurso.cargo}.`
                    : 'VocÃª receberÃ¡ notificaÃ§Ãµes sobre este concurso.'
            });
        }
        refreshIcons();
    ...
    function toggleAvisar(id, btn) {
        ...
        if (isAtivo) {
            ...
            showToast({
                type: 'success',
                title: 'Aviso ativado',
                message: 'VocÃª serÃ¡ notificado assim que o edital for publicado.'
            });
        } else {
            ...
            showToast({
                ...
                message: 'VocÃª nÃ£o receberÃ¡ mais alertas sobre este edital.'
            });
        }
        refreshIcons();
    }
...
if (watchBtn.querySelector('.btn-watch-label')) { toggleAcompanhar(watchBtn.getAttribute('data-watch-id'), watchBtn); }
            ...
            if (detailsBtn) {
                ...
                showToast({
                    ...
                    message: concurso ? `Abrindo detalhes de ${concurso.orgao} â ${concurso.cargo}.` : 'Em breve.'
                });
                return;
            }
        ...
        if (btnNotif) {
            btnNotif.addEventListener('click', () => {
                showToast({
                    ...
                    message: 'VocÃª tem 3 alertas: TCE-SP iminente, novo edital DNIT e prazo encerrando no INCRA.'
                });
            });
        }
    }
    ...
    /* ============================================================
       SUPABASE AUTH â Session check + dropdown
       ============================================================ */
    function buildUserDropdown(user) {
        ...
        <div style="position:relative;">
          <div id="ub" style="display:flex;align-items:center;gap:8px;background:rgba(26,46,74,0.06);border:1px solid #e2e8f0;border-radius:20px;padding:4px 12px 4px 6px;cursor:pointer;">
            ${av}
            <span style="font-size:13px;font-weight:500;color:#1A2E4A;">OlÃ¡, ${primeiro} ð</span>
        ...
        // LOGOUT â limpa localStorage diretamente (signOut via rede falha no plano free)
        document.getElementById('btnSairIndex').addEventListener('click', () => {
            localStorage.clear();
            window.location.reload();
        });

        // Listener ConfiguraÃ§Ãµes da conta (index)
    ...
    (function() {
        ...
        if (p.get('login') === '1') {
            setTimeout(() => {
                modal.open('login');
                showToast({ type: 'info', title: 'Acesso restrito', message: 'FaÃ§a login para acessar sua Ã¡rea.' });
...
/* ===== MODAL CONFIGURAÃÃES DA CONTA â PÃGINA INICIAL ===== */
(function() {
  ...
  <div class="cfg-modal">
    ...
    <div class="cfg-tabs">
      ...
      <button class="cfg-tab" data-cfg-tab="seguranca" onclick="switchCfgTab('seguranca')">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>SeguranÃ§a
      </button>
    ...
    <div class="cfg-body">
      <div class="cfg-panel active" id="cfg-panel-perfil">
        ...
        <p class="cfg-section-title">InformaÃ§Ãµes pessoais</p>
        <div class="cfg-field"><label>Nome completo</label><input type="text" id="cfgNome" value="${nome}" placeholder="Seu nome completo"></div>
        <div class="cfg-field"><label>E-mail atual</label><input type="email" id="cfgEmailAtual" value="${user.email||''}" disabled><small>Para alterar o e-mail, acesse a aba SeguranÃ§a.</small></div>
        <div class="cfg-field"><label>Ãrea de atuaÃ§Ã£o</label><input type="text" id="cfgArea" value="${meta.area||''}" placeholder="Ex: Engenharia Civil"></div>
      </div>
      <div class="cfg-panel" id="cfg-panel-seguranca">
        <p class="cfg-section-title">Alterar e-mail</p>
        <div class="cfg-field"><label>Novo e-mail</label><input type="email" id="cfgNovoEmail" placeholder="novo@email.com"><small>Um link de confirmaÃ§Ã£o serÃ¡ enviado para o novo endereÃ§o.</small></div>
        ...
        <div class="cfg-field"><label>Nova senha</label><input type="password" id="cfgNovaSenha" placeholder="MÃ­nimo 6 caracteres" oninput="cfgPwdStrength(this.value)"><div class="cfg-pwd-strength"><div class="cfg-pwd-bar" id="cfgPwdBar"></div></div><small id="cfgPwdTxt" style="font-weight:500;margin-top:4px;"></small></div>
      ...
      <div class="cfg-panel" id="cfg-panel-pagamento">
        <div class="cfg-plan-info"><div style="width:36px;height:36px;border-radius:50%;background:#e2e8f0;display:flex;align-items:center;justify-content:center;font-size:18px;">ð</div><div><strong style="display:block;font-size:13px;font-weight:700;color:#1A2E4A">Plano Gratuito</strong><span style="font-size:11px;color:#64748b">Acesso aos editais pÃºblicos e controle de concursos</span></div></div>
        <div class="cfg-premium-box"><h4>â¨ Engs Hub Premium â em breve</h4><p>Alertas instantÃ¢neos por e-mail, filtros avanÃ§ados, histÃ³rico completo de bancas e muito mais.</p><button class="cfg-btn-premium" onclick="showToast({type:'success',title:'Em breve!',message:'VocÃª serÃ¡ avisado quando o Premium for lanÃ§ado.'})">â­ Quero ser avisado</button></div>
        <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:14px 16px;"><p style="margin:0;font-size:12px;color:#92400e;line-height:1.5;"><strong>ð³ Forma de pagamento</strong><br>A integraÃ§Ã£o com plataforma de pagamento serÃ¡ disponibilizada em breve.</p></div>
      </div>
    </div>
    <div class="cfg-footer" id="cfgFooter">
      <button class="cfg-btn-cancel" onclick="closeCfg()">Cancelar</button>
      <button class="cfg-btn-save" id="cfgBtnSalvar" onclick="cfgSalvarPerfil()">Salvar alteraÃ§Ãµes</button>
    </div>
  </div>
</div>`;
...
window.cfgPwdStrength = function(s) {
    ...
    const lv=[{w:'20%',c:'#ef4444',l:'Muito fraca'},{w:'40%',c:'#f97316',l:'Fraca'},{w:'60%',c:'#eab308',l:'RazoÃ¡vel'},{w:'80%',c:'#22c55e',l:'Forte'},{w:'100%',c:'#16a34a',l:'Muito forte'}][Math.max(0,sc-1)];
...
window.handleCfgFoto = function(event) {
    ...
    if (!file) return;
    if (file.size > 2*1024*1024) { showToast({type:'warning',title:'Foto muito grande',message:'MÃ¡ximo 2MB.'}); return; }
...
window.cfgSalvarPerfil = async function() {
    ...
    btn.textContent = 'Salvar alteraÃ§Ãµes'; btn.disabled = false;
    ...
    if (nameEl && nameEl.textContent.includes('OlÃ¡')) nameEl.textContent = 'OlÃ¡, '+primeiro+' ð';
    const ddName = document.getElementById('user-greeting-wrap');
    if (ddName) {
        const nameSpan = ddName.querySelector('span[style*="font-size:13px"]');
        if (nameSpan) nameSpan.textContent = 'OlÃ¡, '+primeiro+' ð';
    }
    showToast({type:'success',title:'â Dados atualizados!'});
    closeCfg();
};
...
window.cfgSalvarEmail = async function() {
    ...
    if (!e || !/^[^@]+@[^@]+.[^@]+$/.test(e)) { showToast({type:'warning',title:'E-mail invÃ¡lido.'}); return; }
    btn.textContent='Enviando...';btn.disabled=true;
    const res = await db.auth.updateUser({ email: e });
    btn.textContent='Alterar e-mail';btn.disabled=false;
    if(res.error){showToast({type:'warning',title:'Erro: '+res.error.message});return;}
    showToast({type:'success',title:'â ConfirmaÃ§Ã£o enviada para '+e});
...
window.cfgSalvarSenha = async function() {
    ...
    if(!nova||nova.length<6){showToast({type:'warning',title:'Senha mÃ­n. 6 caracteres.'});return;}
    if(nova!==conf){showToast({type:'warning',title:'Senhas nÃ£o coincidem.'});return;}
    ...
    if(res.error){showToast({type:'warning',title:'Erro: '+res.error.message});return;}
    showToast({type:'success',title:'â Senha alterada!'});
